...
if  nargin > 5
   ...
   %---------------------------------------------------------------
%    options=optimoptions('particleswarm','SwarmSize',100,'MaxIterations',100,'MaxStallIterations',100,'Display','off');
%    infill_criterion = @(theta)objfunc(theta, par);
%    theta =  particleswarm(infill_criterion, n, lob, upb,options);
...
dmodel = struct('regr',regr, 'corr',corr, 'theta',theta', ...
    ...
    'C',fit.C, 'Ft',fit.Ft, 'G',fit.G,'obj',f);
